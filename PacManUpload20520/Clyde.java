...
/**
 * Unlike every other ghost, Clyde is non-confrentational.. He will follow you around but when he gets two close, 
 * he changes his mind and turns around This class extends Sprite and uses class PacMan to acquire 
...
public class Clyde extends Sprite {
	 ...
	 * @param wall
	 */
	public Clyde(PacMan pacman, PacWall wall) {
		try {
			img = ImageIO.read(new File("Clyde.png"));
		} catch (IOException e) {
			e.printStackTrace();
		}
		xAxis = 267;
	...
	/**
	 * Clyde will head for your location, but when it comes within 8 blocks from you, it will get scared and run away. 
	 * I find this by using the distance formula. 
	 * It will keep going forward until it hits a wall; from there, it will 
	...
	public boolean makeNextMove() {
		...
		if (Math.sqrt((Math.pow(targetx - xAxis, 2) + (Math.pow(targety - yAxis, 2)))) <= 8) {
			targetx = 640;
			targety = 20;
		}
