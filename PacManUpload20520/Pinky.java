...
/**
 * Pinky is not your main enemy, but she is still very dangerous... Perhaps even more than Pinky
 * This class extends Sprite and uses class PacMan to acquire 
...
public class Pinky extends Sprite {
	 ...
	 * @param wall
	 */
	public Pinky(PacMan pacman, PacWall wall) {
		try {
			img = ImageIO.read(new File("Pinky.png"));
		} catch (IOException e) {
			e.printStackTrace();
		}
		xAxis = 274;
	...
	/**
	 * With the Graphics2D object, it will paint Pinky at its location with size 20 by 20 (width and height)
	 * @param g2d
	 */
	...
	/**
	 * Unlike Pinky, Pinky will try to ambush you by cutting in front of you. It will keep going forward 
	 * until it hits a wall; from there, it will figure whether turning left or right is the quickest way 
	 * to get to PacMan. If only one side is clear, it will go that side. It returns true if Pinky hit PacMan
	 * @return
	 */
	public boolean makeNextMove() {
		int targetx = pacMan.getxAxis() + 3;
		int targety = pacMan.getyAxis() + 3;
